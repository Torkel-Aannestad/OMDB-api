...
func (app *application) routes() http.Handler {
	...
	router.HandlerFunc(http.MethodPost, "/v1/auth/change-password", app.changePasswordHandler)

	// router.Handler(http.MethodGet, "/metrics", expvar.Handler())

	//Admin swap permission
	router.HandlerFunc(http.MethodPost, "/v1/users/permissions/:id", app.protectedRoute("movies:write", app.addUserPermissionsHandler))
